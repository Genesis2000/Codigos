...
//Carnet: VS18009
//El codigo esta hecho para saber los datos de un empleado y calcular los descuentos de su salario 

#include <iostream>//libreria estandar
#include <cstdlib>//contiene funciones para la gestion de memoria dinamica


using namespace std;//permite abreviar el codigo
...
float calisss(float sal,float tope)//funcion para el calculo del isss
//declaracion de variables
{float isss=0;
	if(sal>=1000)
	isss=30;
	...
	isss=sal*0.03;
return isss;//retorna a la variable isss
}
float calafp(float salario, float afp)//funcion para calcular el afp
{
	afp=salario*0.0725;
	return afp;//returna la variable afp
}
float calrenta(float sal)//funcion para calcular la renta
{
	float renta;//declaracion de variable
	...
	return renta;//retorna la variable renta
}

float calliq(float sal, float isss, float afp, float renta){//funcion para calcular el salario liquido
	
	float salliqui;//declaracion de variable
	
	salliqui=sal-(isss+afp+renta);
	
	return salliqui;//retorna a la variable saldo liquido
}
